...
ChatPanel::ChatPanel(QWidget* parent) : QWidget(parent) {
    ...
    // barra de formatação (BBCode) — botões B I U, cor, tamanho, emoji
    QToolBar* fmt = new QToolBar(this);
    fmt->setIconSize(QSize(16, 16));
    fmt->setFloatable(false);
    fmt->setMovable(false);
    fmt->setStyleSheet(QStringLiteral("QToolBar { border: none; spacing: 1px; }"));
    ...
    QToolButton* minus = btn(QStringLiteral("A⁻"), tr("Reduzir tamanho do texto"));
    QToolButton* plus  = btn(QStringLiteral("A⁺"), tr("Aumentar tamanho do texto"));
    ...
    QToolButton* color = btn(QString(), tr("Cor do texto [color=]"));
    color->setIcon(HIcons::design());
    QToolButton* emoji = btn(QStringLiteral("☺"), tr("Inserir emoticon"));
    QToolButton* clear = btn(QStringLiteral("Tx"), tr("Limpar formatação"));
    ...
    auto wrap = [this](const QString& open, const QString& close) {
        ...
        QString sel = m_input->selectedText();
        if (sel.isEmpty()) {
    ...
    auto mkBrowser = [this]() -> QTextBrowser* {
        return qobject_cast<QTextBrowser*>(m_tabs->currentWidget());
    };
    connect(minus, &QToolButton::clicked, this, [=] {
        if (QTextBrowser* b = mkBrowser()) b->zoomOut();
    });
    connect(plus, &QToolButton::clicked, this, [=] {
        if (QTextBrowser* b = mkBrowser()) b->zoomIn();
    });

    connect(color, &QToolButton::clicked, this, [=] {
        QColor c = QColorDialog::getColor(Qt::black, this, tr("Cor do texto"));
        if (c.isValid()) wrap(QStringLiteral("[color=%1]").arg(c.name()), "[/color]");
    });
    ...
    connect(clear, &QToolButton::clicked, this, [=] {
        ...
        t.remove(QRegularExpression(QStringLiteral("\\[/?(b|i|u|s)\\]")));
        t.remove(QRegularExpression(QStringLiteral("\\[/?color[^\\]]*\\]")));
        t.remove(QRegularExpression(QStringLiteral("\\[/?size[^\\]]*\\]")));
        t.remove(QRegularExpression(QStringLiteral("\\[/?url[^\\]]*\\]")));
        m_input->setText(t);
    });

    connect(m_tabs, &QTabWidget::tabCloseRequested, this, [this](int idx) {
        if (idx <= 1) return; // abas do servidor/canal são fixas
...
void ChatPanel::updateSendTarget() {
    // nada visual por ora — o destino é resolvido no envio
}
...
QString ChatPanel::bbToHtml(const QString& text) {
    ...
    h.replace(reUrl, QStringLiteral("<a href=\"\\1\" style=\"color:#2E7FC4\">\\2</a>"));
    static const QRegularExpression reUrl2("\\[url\\](.*?)\\[/url\\]");
    h.replace(reUrl2, QStringLiteral("<a href=\"\\1\" style=\"color:#2E7FC4\">\\1</a>"));
...
static QString stamp() {
    return QStringLiteral("<span style=\"color:#888888\">[%1]</span>")
        .arg(QDateTime::currentDateTime().toString("HH:mm:ss"));
}

void ChatPanel::addServerChat(const QString& user, const QString& text) {
    addLine(m_serverChat,
            QStringLiteral("%1 <b style=\"color:#2E7FC4\">%2</b>: %3")
...
void ChatPanel::addServerSystem(const QString& text) {
    addLine(m_serverChat,
            QStringLiteral("%1 <i style=\"color:#666666\">* %2</i>")
